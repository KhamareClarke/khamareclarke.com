/**
 * GET /api/empire/seo-runs/list?projectId=myapproved
 * Returns SEO runs from DB (like leads list) for dashboard.
 */
...
export async function GET(req) {
  ...
  try {
    const [runsRes, count24Res] = await Promise.all([
      client.from('empire_seo_runs').select('*').eq('project_id', projectId).order('created_at', { ascending: false }).limit(limit),
      client.from('empire_seo_runs').select('id', { count: 'exact', head: true }).eq('project_id', projectId).gte('created_at', ts24),
    ]);
    if (runsRes.error) {
