...
export function isInternalOpsLeadQuery(text) {
  ...
  if (/\bleads?\b/.test(t) && /\b(?:this|last|current)\s+(?:week|month)\b/.test(t)) return true;
...
/** Sister-site / fleet project questions — never web search. */
...
function isTellMeAboutWebQuery(text) {
  ...
  if (isInternalOpsProjectQuery(t)) return false;
  ...
  if (/\bprojects?\b/i.test(rest) && /\b(?:our|my|all|fleet|information)\b/i.test(t)) return false;
  return true;
}
...
export function messageNeedsWebSearch(text) {
  ...
  if (/^(status|fleet|leads?|briefing|help|open\s+(fleet|clients|leads|projects?))/i.test(t)) return false;
